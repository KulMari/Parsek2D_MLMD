...
public:
	...
	/** initialize with rotated reconnection system */
	void initOBL(VirtualTopology *vct, Grid *grid);
	...
	/** add an amount of tagged charge density to charge density field at node X,Y,Z */
	void addRhotag(double ***weight, int X, int Y,int Z, int is);
	...
	double**** getRHOns() ;
	/** SPECIES: get tagged density array on nodes*/
	double**** getRHOnstag() ;
...
private:
	...
	/*********************************************************************************
		...
		double****  rhons;
		/** SPECIES: tagged charge density for each species, defined on nodes */
		double****  rhonstag;
...
inline void EMfields::calculateField(Grid *grid, VirtualTopology *vct){
	...
	PoissonCorrection = 1; //--- Uncomment not to do the divergence cleaning
//	PoissonCorrection = 0; //--- Uncomment not to do the divergence cleaning
	if (PoissonCorrection){
		...
		if(vct->getYright_neighbor()==-1 && (bcPHIfaceYright ==0 ||  bcPHIfaceYright ==1)) // perfect conductor
			bcPHI_Right(PHI,1);
		// end adjust boundaries before calculating the lap
...
inline void EMfields::PoissonImage(double *image, double *vector, Grid *grid, VirtualTopology *vct){
	...
	if(vct->getYright_neighbor()==-1 && (bcPHIfaceYright ==0 ||  bcPHIfaceYright ==1)) // perfect conductor
		bcPHI_Right(temp,1);
	// if double periodic set Phi to 0.0 on BC
	if (vct->getPERIODICX() && vct->getPERIODICY() && vct->getCartesian_rank() ==0){
		temp[0][0][0] = 0.0;
	}
...
inline void EMfields::init(VirtualTopology *vct, Grid *grid){
	...
	if (restart1==0){
		...
		MPI_Barrier(vct->getCommunicator());
		if (vct->getCartesian_rank()==0)
...
//---		dataset_id = H5Dopen(file_id,"/fields/By/cycle_0");
		dataset_id = H5Dopen1(file_id,"/fields/By/cycle_0");   //--- HDF5 1.8
		status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL,
						 H5S_ALL,H5P_DEFAULT,temp_storage);
		k=0;
		for (int i=1; i < nxn-1; i++)
...
//---		dataset_id = H5Dopen(file_id,"/fields/Bz/cycle_0");
		dataset_id = H5Dopen1(file_id,"/fields/Bz/cycle_0");   //--- HDF5 1.8
		status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL,
						 H5S_ALL,H5P_DEFAULT,temp_storage);
		k=0;
		for (int i=1; i < nxn-1; i++)
...
//---		dataset_id = H5Dopen(file_id,"/fields/Ex/cycle_0");
		dataset_id = H5Dopen1(file_id,"/fields/Ex/cycle_0");   //--- HDF5 1.8
		status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL,
						 H5S_ALL,H5P_DEFAULT,temp_storage);
		k=0;
		for (int i=1; i < nxn-1; i++)
...
//---		dataset_id = H5Dopen(file_id,"/fields/Ey/cycle_0");
		dataset_id = H5Dopen1(file_id,"/fields/Ey/cycle_0");   //--- HDF5 1.8
		status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL,
						 H5S_ALL,H5P_DEFAULT,temp_storage);
		k=0;
		for (int i=1; i < nxn-1; i++)
...
//---		dataset_id = H5Dopen(file_id,"/fields/Ez/cycle_0");
		dataset_id = H5Dopen1(file_id,"/fields/Ez/cycle_0");   //--- HDF5 1.8
		status = H5Dread(dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL,
						 H5S_ALL,H5P_DEFAULT,temp_storage);
		k=0;
		for (int i=1; i < nxn-1; i++)
 ...
 */
inline void EMfields::initOBL(VirtualTopology *vct, Grid *grid){
        double pertGEM;
	...
	if (restart1 ==0){
		...
		for (int i=0; i <nxn; i++)
			for (int j=0; j <nyn; j++){
				...
				Ez[i][j][0] =  0.0;
				// Magnetic field
				Bxn[i][j][0] = B0x*tanh((grid->getYN(i,j,0) - Ly/2)/delta)*cos(theta)
						       +B0z*sin(theta);
				...
				// guide field
				Bzn[i][j][0] = B0z*cos(theta)-B0x*tanh((grid->getYN(i,j,0) - Ly/2)/delta)*sin(theta);
			}
				// communicate ghost
				communicateNode(nxn,nyn,Bxn,vct);
		communicateNode(nxn,nyn,Byn,vct);
		...
		for (int i=0; i <nxc; i++)
			for (int j=0; j <nyc; j++){
				Bxc[i][j][0] = B0x*tanh((grid->getYC(i,j,0) - Ly/2)/delta)*cos(theta)+B0z*sin(theta);
				// add the initial GEM perturbation
				...
				// guide field
				Bzc[i][j][0] = B0z*cos(theta)-B0x*tanh((grid->getYC(i,j,0) - Ly/2)/delta)*sin(theta);
			}
				communicateCenter(nxc, nyc,Bxc,vct);
		communicateCenter(nxc, nyc,Byc,vct);
		communicateCenter(nxc, nyc,Bzc,vct);
	...
	if (restart1 ==0){
		...
		for (int i=0; i <nxn; i++)
			for (int j=0; j <nyn; j++){
				...
				// guide field
				Bzn[i][j][0] = B0z + B0x/cosh((grid->getYN(i,j,0) - Ly/2)/delta);
			}
				// communicate ghost
		//		communicateNode(nxn,nyn,Bxn,vct);
		//communicateNode(nxn,nyn,Byn,vct);
		...
		for (int i=0; i <nxc; i++)
			for (int j=0; j <nyc; j++){
				...
				// guide field
				Bzc[i][j][0] = B0z + B0x/cosh((grid->getYC(i,j,0) - Ly/2)/delta);
			}
		//communicateCenter(nxc, nyc,Bxc,vct);
		//communicateCenter(nxc, nyc,Byc,vct);
		//communicateCenter(nxc, nyc,Bzc,vct);
...
inline void EMfields::communicateGhostP2G(int ns,int bcFaceXright, int bcFaceXleft, int bcFaceYright, int bcFaceYleft, VirtualTopology *vct){
	...
	communicateInterp(nxn,nyn,ns,rhonstag,0,0,0,0,dx,dy,vct);
	...
	communicateNode(nxn, nyn, rhonstag, ns,vct);
...
/** add an amount of tagged charge density to charge density field at node X,Y */
inline void EMfields::addRhotag(double ***weight, int X, int Y, int Z, int is){
        rhonstag[is][X-1][Y-1][0] += weight[0][0][0]*invVOL;
        rhonstag[is][X-1][Y][0]   += weight[0][1][0]*invVOL;
        rhonstag[is][X][Y-1][0]   += weight[1][0][0]*invVOL;
        rhonstag[is][X][Y][0]     += weight[1][1][0]*invVOL;
}
/** add an amount of charge density to current density - direction X to 
current density field on the node*/
	...
	switch(dir){
		...
		case 1: // boundary condition on Y-DIRECTION left
			...
			for (int i=0; i < nxc;i++){
				...
				Bxc[i][0][0] = B0x*tanh((grid->getYC(i,0,0) - Ly/2)/delta)*cos(theta)+B0z*sin(theta);
				//Bxc[i][1][0] = B0x*tanh((grid->getYC(i,0,0) - Ly/2)/delta);
				//Bxc[i][2][0] = B0x*tanh((grid->getYC(i,0,0) - Ly/2)/delta);
				Byc[i][0][0] = B0y;
				Bzc[i][0][0] = B0z*cos(theta)-B0x*tanh((grid->getYC(i,0,0) - Ly/2)/delta)*sin(theta);///cosh((grid->getYC(i,0,0) - Ly/2)/delta);
				//Bzc[i][0][0] = B0z + B0x/cosh((grid->getYC(i,0,0) - Ly/2)/delta);
				Bzc[i][1][0] = Bzc[i][0][0];
				Bzc[i][2][0] = Bzc[i][0][0];
    ...
    switch(dir){
		...
		case 1: // boundary condition on Y-DIRECTION left
			for (int i=0; i < nxc;i++){
				...
				Bxc[i][nyc-1][0] = B0x*tanh((grid->getYC(i,nyc-1,0) - Ly/2)/delta)*cos(theta)+B0z*sin(theta);
				//Bxc[i][nyc-2][0] = B0x*tanh((grid->getYC(i,nyc-1,0) - Ly/2)/delta);
				//Bxc[i][nyc-3][0] = B0x*tanh((grid->getYC(i,nyc-1,0) - Ly/2)/delta);
				Byc[i][nyc-1][0] = B0y;
				Bzc[i][nyc-1][0] = B0z*cos(theta)-B0x*tanh((grid->getYC(i,nyc-1,0) - Ly/2)/delta)*sin(theta);///cosh((grid->getYC(nyc-1,0,0) - Ly/2)/delta);;
				//Bzc[i][nyc-1][0] = B0z + B0x/cosh((grid->getYC(nyc-1,0,0) - Ly/2)/delta);;
...
inline void EMfields::bcPHI_Left(double ***im,int dir){
	switch(dir){
		case 0:
			for (int i=0; i <  nyc;i++){
				im[0][i][0] = 0.0;		
//				im[0][i][0] = im[1][i][0];	//--- no grad, so E=0 at boundary
			}
			break;
		case 1:
			for (int i=1; i < nxc;i++){
...
inline void EMfields::bcPHI_Right(double ***im,int dir){
	switch(dir){
		case 0:
			for (int i=0; i <  nyc;i++){
				im[nxc-1][i][0] = 0.0;			//--- impose 0 
//				im[nxc-1][i][0] = im[nxc-2][i][0];	//--- no grad, so E=0 at boundary
			}
			break;
		case 1:
			for (int i=0; i < nxc;i++){
...
/** get tagged density array defined on nodes*/
inline double**** EMfields::getRHOnstag() {return(rhonstag);}
...
inline EMfields::EMfields(CollectiveIO *col,Grid *grid){
	...
	rhonstag = newArr4(double,ns,nxn,nyn,1);
