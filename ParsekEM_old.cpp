...
int main (int argc, char **argv) {
 ...
 int P2Gops=1;
 int PRASendRcvOps=1; 
 int PRACollectionMethod=0;   /* =0, coarse particle to be used for repopulation collected one by one during communicate ops; WORKING ON
				    =1, coarse particles collected all together after mover */
 int RefLevelAdj=1; /* options:                                                                      
...
/******************************************************/
  ...
  cout << "Fields initializations done"<<endl;
  //EMf->initLightwave(vct,grid); // initialize with a dipole
  
  EMf->initWeightBC(vct,grid,col);// Initialize the weight for interpolation of BC between grids
  EMf->initWeightProj(vct,grid,col);// Initialize the weight for projection of refined fields between grids
  cout << "Weights initializations done"<<endl;
  ...
  for (int cycle = first_cycle; cycle < (col->getNcycles()+first_cycle); cycle++)
  {
    ...
    if (myrank==0)
    {
      cout << "Barrier after particle mover/send/receive, cycle " << cycle+1 << endl;
      }*/
