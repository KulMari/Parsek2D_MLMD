...
public:
	...
	void AddPerturbation(double deltaBoB, double kx, double ky, double Ex_mod, double Ex_phase, double Ey_mod, double Ey_phase, double Ez_mod, double Ez_phase, double Bx_mod, double Bx_phase, double By_mod, double By_phase, double Bz_mod, double Bz_phase, double B0, Grid *grid,VirtualTopology *vct);
        ...
        void initWeightBC(VirtualTopology *vct, Grid *grid, CollectiveIO* col );
        /** Initialize the weights for projections of refined fields between grids **/
        void initWeightProj(VirtualTopology *vct, Grid *grid, CollectiveIO* col );
	...
	if (PoissonCorrection){
		...
		if (grid->getLevel()==0)
		...
		else
		  {
		    ...
		    GMRES(&Field::PoissonImage, xkrylovPoisson, (nxc)*(nyc),bkrylovPoisson,20,200,GMREStol, grid, vct, this);
		...
		// adjust boundaries before calculating the lap
		  if(vct->getXleft_neighbor()==MPI_PROC_NULL && (bcPHIfaceXleft ==0 ||  bcPHIfaceXleft ==1)) // perfect conductor
		    bcPHI_Left(PHI,0);
		  // boundary condition: Xright
		  if(vct->getXright_neighbor()==MPI_PROC_NULL && (bcPHIfaceXright ==0 ||  bcPHIfaceXright ==1)) // perfect conductor
		    bcPHI_Right(PHI,0);
		  // boundary condition: Yleft
		  if(vct->getYleft_neighbor()==MPI_PROC_NULL && (bcPHIfaceYleft ==0 ||  bcPHIfaceYleft ==1)) // perfect conductor
		    bcPHI_Left(PHI,1);
		// boundary condition: Yright
		  if(vct->getYright_neighbor()==MPI_PROC_NULL && (bcPHIfaceYright ==0 ||  bcPHIfaceYright ==1)) // perfect conductor
		    bcPHI_Right(PHI,1);
                }
...
inline void EMfields::initWeightBC(VirtualTopology *vct, Grid *grid, CollectiveIO* col){
...
// If the x=Ox+finelx+finedx ghost nodes of the fine grid intersect this process
    ...
    /*for (i=0;i<nmessageBC;i++){
      cout << "sending BC "<<npointssent[i]<<" points to "<<targetBC[i]<<endl;
    }*/
    // AMR, ME
    if (0)
      {
	cout << "nmessageBC: " <<nmessageBC <<", sum targets: " << targetBOTTOM+ targetTOP+ targetLEFT+ targetRIGHT <<endl;

	cout << "From inside Fields:" <<endl;
	for (int i=0; i< nmessageBC; i++)
	  {
	    cout << "i: " <<i << ", targetBC[i] "<<targetBC[i] <<", BCSide[i] " <<BCSide[i]<<endl;
	  }
	cout << "targetBOTTOM " << targetBOTTOM << ", targetTOP " << targetTOP << ", targetLEFT " << targetLEFT << ", targetRIGHT " << targetRIGHT <<endl;
      }
    // end AMR, ME
}
...
if ( grid->getLevel() > 0) {                             //If this grid is considered as fine by another grid
    ...
    /*for (i=0;i<nmessagerecuBC;i++){
      cout <<  ": receiving BC "<<npointsreceived[i]<<" points from "<<fromBC[i]<<endl;
    }*/
...
int X,Y,i,j,k,ix,iy,nproc,nybloc,nxmrecv,nymrecv;
double coarsedx,coarsedy,coarselx,coarsely,finelx,finely,Ox,Oy,finedx,finedy;
double xloc, yloc,xfirst,xlast,yfirst,ylast,xfirstnext,yfirstnext,xstop,ystop;
...
if ( grid->getLevel() > 0) {                             //If this grid is considered as fine by another grid
    ...
    xfirst = ceil((grid->getXstart()+Ox)/coarsedx)*coarsedx - Ox;//Coordinate of the first point in x direction of the coarse grid intersecting this proc expressed in the fine grid frame
    X= floor((Ox+xfirst)/coarsedx)/(grid->getNXC()-2); // X coordinate of the proc of the coarse grid intersecting the origin of this proc   
    yfirst = ceil((grid->getYstart()+Oy)/coarsedy)*coarsedy - Oy;//Coordinate of the first point in x direction of the coarse grid intersecting this proc expressed in the fine grid frame
    Y= floor((Oy+yfirst)/coarsedy)/(grid->getNYC()-2); // X coordinate of the proc of the coarse grid intersecting the origin of this proc   
    ...
    xlast = floor((grid->getXend()+Ox)/coarsedx)*coarsedx - Ox;
    ylast = floor((grid->getYend()+Oy)/coarsedy)*coarsedy - Oy;
    xfirstnext = ceil((grid->getXend()+Ox)/coarsedx)*coarsedx - Ox;
    yfirstnext = ceil((grid->getYend()+Oy)/coarsedy)*coarsedy - Oy;
    if (xlast == xfirstnext && vct->getCoordinates(0)!=vct->getXLEN()-1){
        xlast = xlast - coarsedx;
    }
    if (ylast == yfirstnext && vct->getCoordinates(1)!=vct->getYLEN()-1){
        ylast = ylast - coarsedy;
    }
    xstop = (X+1)*(grid->getNXC()-2)*coarsedx-Ox; //End of domain of processor whith coordinate X
    ystop = (Y+1)*(grid->getNYC()-2)*coarsedy-Oy; //End of domain of processor with coordinate Y
    ...
    for (i =0;i<nxmsend+nxpsend;i++) {
        xloc = xfirst + i * coarsedx;
        ix = 2 +  int(floor((xloc-grid->getXstart())/grid->getDX()));
        ixsentProj[i]=ix;
        for (j =0;j<nymsend+nypsend;j++) {
            yloc = yfirst + j * coarsedy;
            iy = 2 +  int(floor((yloc-grid->getYstart())/grid->getDY()));
            iysentProj[j]=iy;
            weightProj[i][j][3][0] = ((xloc - grid->getXN(ix-1,iy-1,0))/grid->getDX())*((yloc - grid->getYN(ix-1,iy-1,0))/grid->getDY()); // weight +:+
            weightProj[i][j][2][0] = ((xloc - grid->getXN(ix-1,iy,0))/grid->getDX())*((grid->getYN(ix-1,iy,0) - yloc)/grid->getDY()); //weight +:-
            weightProj[i][j][1][0] = ((grid->getXN(ix,iy-1,0) - xloc)/grid->getDX())*((yloc - grid->getYN(ix,iy-1,0))/grid->getDY()); // weight -:+
            weightProj[i][j][0][0] = ((grid->getXN(ix,iy,0) - xloc)/grid->getDX())*((grid->getYN(ix,iy,0) - yloc)/grid->getDY()); // weight -:-
        }
    }
    //cout<< vct->getCartesian_rank_COMMTOTAL() << " sends to target0 = " << targetProj[0]<<" nmessageProj= "<<nmessageProj<<" nxmsend= "<<nxmsend<<" nxpsend= "<<nxpsend<<" nymsend= "<<nymsend<<" nypsend= "<<nypsend<< "xfirst = "<<xfirst<<"xlast = "<<xlast<<" yfirst= "<<yfirst<<" ylast = "<<ylast<<endl;
...
if (grid->getLevel() < vct->getNgrids()-1) {
    ...
    finedx = grid->getDX()/col->getRatio();
    finedy = grid->getDY()/col->getRatio();
    if(grid->getXstart()<Ox+coarselx && grid->getXend()>Ox && grid->getYstart()<Oy+coarsely && grid->getYend()>Oy){
        xfirst = max(ceil(Ox/grid->getDX())*grid->getDX()-Ox,grid->getXstart()-Ox);//x of the first point of this proc intersecting the finer domain (in the fine domain frame) 
        yfirst = max(ceil(Oy/grid->getDY())*grid->getDY()-Oy,grid->getYstart()-Oy);//y of the first point of this proc intersecting the finer domain (in the fine domain frame)
        ...
        xlast = min(floor((Ox+finelx)/grid->getDX())*grid->getDX()-Ox,grid->getXend()-xstop-Ox);//x of the last point of this proc intersecting the finer domain (in the fine domain frame)
        ylast = min(floor((Oy+finely)/grid->getDY())*grid->getDY()-Oy,grid->getYend()-ystop-Oy);//y of the last point of this proc intersecting the finer domain (in the fine domain frame)
        nxmrecv=floor((xlast-xfirst)/grid->getDX()+0.5)+1;
        nymrecv=floor((ylast-yfirst)/grid->getDY()+0.5)+1;
        nybloc=0;
        for (i=0;i<nxmrecv;i++){
            for (j=0;j<nymrecv;j++){
                xloc = xfirst + i * grid->getDX();
                yloc = yfirst + j * grid->getDY();
                nproc = min(floor(yloc/((grid->getNYC()-2.)*finedy)),col->getYLEN()-1.)+col->getYLEN()*min(floor(xloc/((grid->getNXC()-2.)*finedx)),col->getXLEN()-1.)+col->getXLEN()*col->getYLEN()*(grid->getLevel()+1); // rank of the proc on the fine grid sending this point(in MPI_COMM_WORLD)
                k=0;
                while(nproc != fromProj[k] && k < nmessagerecvProj){
                    k++;
                }
                if(k == nmessagerecvProj){
                    nmessagerecvProj++;
                    fromProj[k]=nproc;
                    npointsreceivedProj[k]=0;
                    ixrecvfirstProj[k]=floor((xloc+Ox-grid->getXstart())/grid->getDX()+0.5)+1;
                    iyrecvfirstProj[k]=floor((yloc+Oy-grid->getYstart())/grid->getDY()+0.5)+1;
                    if (i==0) {
                        nybloc++;
                    }
                }
                    npointsreceivedProj[k]++;
                    nxrecvProj[k]=i+1;
                    nyrecvProj[k]=j+1;
            }
        }        
        for (k=nmessagerecvProj-1;k>0;k=k-1){
             X = k/nybloc;
             Y = k - X*nybloc;
             if(Y > 0) {
                 nyrecvProj[k] -= nyrecvProj[k-1];
             }
             if(X > 0) {
                 nxrecvProj[k] -= nxrecvProj[k-nybloc];
             }
        }
    }
    /*for (k=0;k<nmessagerecvProj;k++){
        cout << "Proc " << vct->getCartesian_rank_COMMTOTAL() <<"receives from " << fromProj[k]<<" npoints received = "<<npointsreceivedProj[k]<<endl;
    }*/
}
}
/** communicate ghost for grid -> Particles interpolation */
	...
	switch(dir){
		case 0: // boundary condition on X-DIRECTION left
			for (int i=1; i <  nyn-1;i++){
				Bxn[0][i][0] = 0.0;
				Byn[0][i][0] = (Byc[0][i-1][0]+Byc[0][i][0])/2.;
				Bzn[0][i][0] = (Bzc[0][i-1][0]+Bzc[0][i][0])/2.;
			}
				Bxn[0][0][0] = 0.0;
				Byn[0][0][0] = Byc[0][0][0];
				Bzn[0][0][0] = Bzc[0][0][0];
				Bxn[0][nyn-1][0] = 0.0;
				Byn[0][nyn-1][0] = Byc[0][nyc-1][0];
				Bzn[0][nyn-1][0] = Bzc[0][nyc-1][0];
 
			break;
		case 1: // boundary condition on Y-DIRECTION left
                  	for (int i=1; i < nxn-1;i++){
				Bxn[i][0][0] = (Bxc[i][0][0]+Bxc[i-1][0][0])/2.;
				Byn[i][0][0] = 0.0;         
				Bzn[i][0][0] = (Bzc[i][0][0]+Bzc[i-1][0][0])/2.;
			}
				Byn[0][0][0] = 0.0;
				Bxn[0][0][0] = Bxc[0][0][0];
				Bzn[0][0][0] = Bzc[0][0][0];
...
inline  void EMfields::BperfectConductorRight(int dir,Grid *grid,VirtualTopology *vct){
	switch(dir){
		case 0: // boundary condition on X-DIRECTION right
			for (int i=1; i <  nyn-1;i++){                        
				Bxn[nxn-1][i][0] = 0.0;                     
				Byn[nxn-1][i][0] = (Byc[nxn-2][i][0]+Byc[nxn-2][i-1][0])/2.;
				Bzn[nxn-1][i][0] = (Bzc[nxn-2][i][0]+Bzc[nxn-2][i-1][0])/2.;
			}
				Bxn[nxn-1][0][0] = 0.0;                     
		...
		case 1: // boundary condition on Y-DIRECTION right
                       for (int i=1; i < nxn-1;i++){                          
				Bxn[i][nyn-1][0] = (Bxc[i][nyn-2][0]+Bxc[i-1][nyn-2][0])/2.;
				Byn[i][nyn-1][0] = 0.0;             
				Bzn[i][nyn-1][0] = (Bzc[i][nyn-2][0]+Bzc[i-1][nyn-2][0])/2.;
			}
	   ...
	   switch(dir){
		   case 0: // boundary condition on X-DIRECTION left
			   for (int i=0; i <  nyn;i++){
				   ...
				   vectorX[0][i][0] =  vectorX[1][i][0];// - rhon[1][i][0]*dy;
				   vectorY[0][i][0] =  0.0;
				   vectorZ[0][i][0] =  0.0;
			   }
			   break;
		   case 1: // boundary condition on Y-DIRECTION left
			   ...
			   for (int i=0; i < nxn;i++){
				   ...
				   vectorY[i][0][0] = (Ey[i][1][0] - susxy[i]*vectorX[i][0][0] -susyz[i]*vectorZ[i][0][0] -Jyh[i][1][0]*dt*th)/susyy[i];
	                   ...
	                   delete[] suszz;*/
			   for (int i=0; i < nxn;i++){
			       vectorY[i][0][0] = vectorY[i][1][0];// - rhon[i][1][0]*dx;
			       vectorX[i][0][0] =  0.0;
			       vectorZ[i][0][0] =  0.0;
                           }
...
inline  void EMfields::perfectConductorRight(double ***vectorX, double ***vectorY, double ***vectorZ,int dir,Grid *grid){
	   ...
	   switch(dir){
		   case 0: // boundary condition on X-DIRECTION RIGHT
			   for (int i=0; i <  nyn;i++){
                                   //Stefano's version below
				   vectorX[nxn-1][i][0] = vectorX[nxn-2][i][0];//+rhon[nxn-2][i][0]*dy;
		   ...
		   case 1: // boundary condition on Y-DIRECTION RIGHT
			   ...
			   for (int i=0; i < nxn;i++){
				   vectorX[i][nyn-1][0] = 0.0;
				   vectorZ[i][nyn-1][0] = 0.0;
				   vectorY[i][nyn-1][0] = vectorY[i][nyn-2][0];// +rhon[i][nyn-2][0]*dx;
...
if (nmessageProj > 0) {
    for (i=0;i<nxmsend;i++){
        for (j=0;j<nymsend;j++){
            //cout << "Proc " << vct->getCartesian_rank_COMMTOTAL()<< " nxmsend "<<nxmsend << " nymsend "<<nymsend<<" ixproj = "<<ixsentProj[i]<<" iyproj = "<< iysentProj[j]<<endl;
            bufferProj[start  ] = weightProj[i][j][0][0]*Ex[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ex[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ex[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ex[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start+1] = weightProj[i][j][0][0]*Ey[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ey[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ey[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ey[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start+2] = weightProj[i][j][0][0]*Ez[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ez[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ez[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ez[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start+3] = weightProj[i][j][0][0]*Bxn[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Bxn[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Bxn[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Bxn[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start+4] = weightProj[i][j][0][0]*Byn[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Byn[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Byn[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Byn[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start+5] = weightProj[i][j][0][0]*Bzn[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Bzn[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Bzn[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Bzn[ixsentProj[i]][iysentProj[j]][0];
            start += 6;
        }
    }
...
if (nmessageProj > 1 && nxpsend > 0){
    step = start;
    for (i=nxmsend;i<nxmsend+nxpsend;i++){
        for (j=0;j<nymsend;j++){
            bufferProj[start ] = weightProj[i][j][0][0]*Ex[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ex[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ex[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ex[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start +1] = weightProj[i][j][0][0]*Ey[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ey[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ey[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ey[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start +2] = weightProj[i][j][0][0]*Ez[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ez[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ez[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ez[ixsentProj[i]][iysentProj[j]][0];
...
if (nmessageProj > 1 && nypsend > 0){
    step = start;
    for (i=0;i<nxmsend;i++){
        for (j=nymsend;j<nymsend+nypsend;j++){
            bufferProj[start ] = weightProj[i][j][0][0]*Ex[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ex[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ex[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ex[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start +1] = weightProj[i][j][0][0]*Ey[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ey[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ey[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ey[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start +2] = weightProj[i][j][0][0]*Ez[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Ez[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Ez[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Ez[ixsentProj[i]][iysentProj[j]][0];
            bufferProj[start +3] = weightProj[i][j][0][0]*Bxn[ixsentProj[i]-1][iysentProj[j]-1][0]+ weightProj[i][j][1][0]*Bxn[ixsentProj[i]-1][iysentProj[j]][0] + weightProj[i][j][2][0]*Bxn[ixsentProj[i]][iysentProj[j]-1][0] + weightProj[i][j][3][0]*Bxn[ixsentProj[i]][iysentProj[j]][0];
...
for (i=0;i<nmessagerecvProj;i++) {
    ...
    for (j=0;j<nxrecvProj[i];j++){
        for (k=0;k<nyrecvProj[i];k++){
            Ex[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = (Ex[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] + bufferProj[(k+j*nyrecvProj[i])*6]  )/2.;
            Ey[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = (Ey[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] + bufferProj[(k+j*nyrecvProj[i])*6+1])/2.;
            Ez[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = (Ez[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] + bufferProj[(k+j*nyrecvProj[i])*6+2])/2.;
            Bxn[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = bufferProj[(k+j*nyrecvProj[i])*6+3];
            Byn[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = bufferProj[(k+j*nyrecvProj[i])*6+4];
            Bzn[ixrecvfirstProj[i]+j][iyrecvfirstProj[i]+k][0] = bufferProj[(k+j*nyrecvProj[i])*6+5];
        }
    }
}
//Then the coarse grid has to update the overlapping cells
if (Ox <=grid->getXstart() && Ox+finelx >= grid->getXstart()) {
    //Send a message to proc  coorinateX-1
    step=0;
    for (ix=1;ix<3;ix++){
        for(iy=0;iy<nyn;iy++){
            bufferProjsend[step] = Ex[ix][iy][0];
            bufferProjsend[step+1] = Ey[ix][iy][0];
            bufferProjsend[step+2] = Ez[ix][iy][0];
            bufferProjsend[step+3] = Bxn[ix][iy][0];
            bufferProjsend[step+4] = Byn[ix][iy][0];
            bufferProjsend[step+5] = Bzn[ix][iy][0];
            step=step+6;
        }
    }
    if (Ox <=grid->getXend() && Ox+finelx >= grid->getXend()) {
    //also receive a message
        ierr = MPI_Sendrecv(bufferProjsend,12*nyn,MPI_DOUBLE,vct->getXleft_neighbor(),1,bufferProjrecv,12*nyn,MPI_DOUBLE,vct->getXright_neighbor(),1,vct->getCART_COMM(), &status);
    } else {
    //send only
        ierr = MPI_Send(bufferProjsend,12*nyn,MPI_DOUBLE,vct->getXleft_neighbor(),1,vct->getCART_COMM());
    }
} else {
    //Do not send ...
      if (Ox <=grid->getXend() && Ox+finelx >= grid->getXend()) {
      //... but receive only
          ierr = MPI_Recv(bufferProjrecv,12*nyn,MPI_DOUBLE,vct->getXright_neighbor(),1,vct->getCART_COMM(), &status);
      }
}
if (Ox <=grid->getXend() && Ox+finelx >= grid->getXend()) {
    //If a message from proc coorinateX+1 has been received
    step =0; 
    for (ix=nxn-2;ix<nxn;ix++){
        for (iy=0;iy<nyn;iy++){
            Ex[ix][iy][0] = bufferProjrecv[step];
            Ey[ix][iy][0] = bufferProjrecv[step+1];
            Ez[ix][iy][0] = bufferProjrecv[step+2];
            Bxn[ix][iy][0] = bufferProjrecv[step+3];
            Byn[ix][iy][0] = bufferProjrecv[step+4];
            Bzn[ix][iy][0] = bufferProjrecv[step+5];
            step=step+6;
        }
    }
    ...
    if (Ox <=grid->getXstart() && Ox+finelx >= grid->getXstart()) {
    //also receive a message
    ierr = MPI_Sendrecv(bufferProjsend,6*nyn,MPI_DOUBLE,vct->getXright_neighbor(),1,bufferProjrecv,6*nyn,MPI_DOUBLE,vct->getXleft_neighbor(),1,vct->getCART_COMM(), &status);
    } else {
    //send only
    ierr = MPI_Send(bufferProjsend,6*nyn,MPI_DOUBLE,vct->getXright_neighbor(),1,vct->getCART_COMM());
    }
      ...
      //Do not send ...
       if (Ox <=grid->getXstart() && Ox+finelx >= grid->getXstart()) {
       //...receive only
       ierr = MPI_Recv(bufferProjrecv,6*nyn,MPI_DOUBLE,vct->getXleft_neighbor(),1,vct->getCART_COMM(), &status);
       }
}
if (Ox <=grid->getXstart() && Ox+finelx >= grid->getXstart()) {
...
if (Oy <=grid->getYstart() && Oy+finely >= grid->getYstart()) {
    ...
    //also receive
        //cout << vct->getCartesian_rank() << " sends 12 to " << vct->getYleft_neighbor() << "and receives 12 from "<< vct->getYright_neighbor() << endl;
        ierr = MPI_Sendrecv(bufferProjsend,12*nxn,MPI_DOUBLE,vct->getYleft_neighbor(),1,bufferProjrecv,12*nxn,MPI_DOUBLE,vct->getYright_neighbor(),1,vct->getCART_COMM(), &status);
    }else {
    //Send only
        //cout << vct->getCartesian_rank() << " sends 12 to " << vct->getYleft_neighbor() << endl;
        ierr = MPI_Send(bufferProjsend,12*nxn,MPI_DOUBLE,vct->getYleft_neighbor(),1,vct->getCART_COMM());
    }
} else {
    //Do not send ...
    if (Oy <=grid->getYend() && Oy+finely >= grid->getYend()) {
    //... but receive only
        //cout << vct->getCartesian_rank() << " receives 12 from "<< vct->getYright_neighbor() << endl;
        ierr = MPI_Recv(bufferProjrecv,12*nxn,MPI_DOUBLE,vct->getYright_neighbor(),1,vct->getCART_COMM(), &status);
    }
}

if (Oy <=grid->getYend() && Oy+finely >= grid->getYend()) {
    //A message from proc coorinateY+1 has been received
    //cout << vct->getCartesian_rank() << " applies received buffer 12" << endl;
    step=0;
    for (ix=0;ix<nxn;ix++){
        for (iy=nyn-2;iy<nyn;iy++){
            Ex[ix][iy][0]  = bufferProjrecv[step];
            Ey[ix][iy][0]  = bufferProjrecv[step+1];
            Ez[ix][iy][0]  = bufferProjrecv[step+2];  
            Bxn[ix][iy][0] = bufferProjrecv[step+3];  
    ...
    if (Oy <=grid->getYstart() && Oy+finely >= grid->getYstart()) {
    //Also receive
      //  cout << vct->getCartesian_rank() << " sends 6 to " << vct->getYright_neighbor() << "and receives 6 from "<< vct->getYleft_neighbor() << endl;
        ierr = MPI_Sendrecv(bufferProjsend,6*nxn,MPI_DOUBLE,vct->getYright_neighbor(),1,bufferProjrecv,6*nxn,MPI_DOUBLE,vct->getYleft_neighbor(),1,vct->getCART_COMM(), &status);
    }else{
    //Send only
        //cout << vct->getCartesian_rank() << " sends 6 to " << vct->getYright_neighbor() << endl;
        ierr = MPI_Send(bufferProjsend,6*nxn,MPI_DOUBLE,vct->getYright_neighbor(),1,vct->getCART_COMM());
    }
} else {
    //Do not send ...
       if (Oy <=grid->getYstart() && Oy+finely >= grid->getYstart()) {
       //...but receive only  
        //cout << vct->getCartesian_rank() << " receives 6 from "<< vct->getYleft_neighbor() << endl;
           ierr = MPI_Recv(bufferProjrecv,6*nxn,MPI_DOUBLE,vct->getYleft_neighbor(),1,vct->getCART_COMM(), &status);
       }
}
if (Oy <=grid->getYstart() && Oy+finely >= grid->getYstart()) {
    //A message from proc coordinateY-1 has been received
    iy=0;
    //cout << vct->getCartesian_rank() << " applies received buffer 6" << endl;
    for (ix=0;ix<nxn;ix++){
        Ex[ix][iy][0] =  bufferProjrecv[6*ix];
        Ey[ix][iy][0] =  bufferProjrecv[6*ix+1];
        Ez[ix][iy][0] =  bufferProjrecv[6*ix+2];
        Bxn[ix][iy][0] = bufferProjrecv[6*ix+3];
        Byn[ix][iy][0] = bufferProjrecv[6*ix+4];
        Bzn[ix][iy][0] = bufferProjrecv[6*ix+5];
    }
}
...
inline EMfields::EMfields(CollectiveIO *col,Grid *grid, VCtopology *vct){
	...
	nyc = grid->getNYC();
	nyn = grid->getNYN();
        nxnproj = nxc /col->getRatio() +1;
        nynproj = nyc /col->getRatio() +1;
	dx = grid->getDX();
	dy = grid ->getDY();
	...
	iyrecvfirstProj  = new int[col->getXLEN()*col->getYLEN()];
        ...
        ixsentProj = new int[nxnproj];
        iysentProj = new int[nynproj];
	for (int i=0; i < ns;i++)
		qom[i] = col->getQOM(i);
	// GEM challenge parameters
	B0x = col->getB0x();
	...
	weightProj   = newArr4(double,nxnproj,nynproj,4,1);
...
inline EMfields::~EMfields(){
	...
	delArr4(weightProj,nxnproj,nynproj,4);
        delete[] bufferBC;
        delete[] bufferProj;
        delete[] bufferBCExxm ;
        delete[] bufferBCExxp ;
