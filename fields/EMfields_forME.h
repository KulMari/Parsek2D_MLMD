...
if ( grid->getLevel() > 0) {                             //If this grid is considered as fine by another grid
	...
	if (vct->getXright_neighbor()==MPI_PROC_NULL){
        ...
        xlast = ceil((grid->getXend()-grid->getDX()+Ox)/coarsedx)*coarsedx;
	...
	if (vct->getYright_neighbor()==MPI_PROC_NULL){
        ...
        ylast = ceil((grid->getYend()-grid->getDY()+Oy)/coarsedy)*coarsedy;
...
if (grid->getLevel() < vct->getNgrids()-1) {
    ...
    if(grid->getXstart()<Ox+finelx && grid->getXend()>Ox && grid->getYstart()<Oy+finely && grid->getYend()>Oy){
     ...
     nprocfirst = min(floor((yfirst-Oy)/((grid->getNYC()-2.)*finedy)),col->getYLEN()-1.)+col->getYLEN()*min(floor((xfirst-Ox)/((grid->getNXC()-2.)*finedx)),col->getXLEN()-1.)+col->getXLEN()*col->getYLEN()*(grid->getLevel()+1); // rank of the proc on the fine grid sending this point(in MPI_COMM_WORLD)
    nproclast = min(floor((ylast-Oy)/((grid->getNYC()-2.)*finedy)),col->getYLEN()-1.)+col->getYLEN()*min(floor((xlast-Ox)/((grid->getNXC()-2.)*finedx)),col->getXLEN()-1.)+col->getXLEN()*col->getYLEN()*( grid->getLevel()+1); // rank of the proc on the fine grid sending this point(in MPI_COMM_WORLD)
    ...
    cout << vct->getCartesian_rank_COMMTOTAL() << " before correction nproc first = "<<nprocfirst<<" nproc last = "<<nproclast<<" X= "<<X<<" Y= "<<Y << endl;

    if(xlast == grid->getXend() && X+1 != col->getXLEN()){
        nproclast -= col->getYLEN(); 
    }
    if(ylast == grid->getYend() && Y+1 != col->getYLEN()){
        nproclast -= 1; 
    }
   ...
   for (i=0;i<nmessagerecvProj;i++){
       ...
       xlocallast = min(          grid->getXend(), ceil(   (Ox+(Xlocal+1)*finelx/col->getXLEN())/grid->getDX()   ) *  grid->getDX()                            );//First updated point by proc fromProj[i]
       ylocallast = min(          grid->getYend(), ceil(   (Oy+(Ylocal+1)*finely/col->getYLEN())/grid->getDY()   ) *  grid->getDY()                            );//First updated point by proc fromProj[i]
       ...
       cout << vct->getCartesian_rank_COMMTOTAL() << "receiving from "<< fromProj[i] << " number of points = "<<npointsreceivedProj[i]<< endl;
   ...
   if (vct->getCartesian_rank_COMMTOTAL()==14){
       for(j=0;j<nyn;j++){
           for(i=0;i<nxn;i++){
     ...
     /*   nybloc=0;
        for (i=0;i<nxmrecv;i++){
            for (j=0;j<nymrecv;j++){
                xloc = xfirst + i * grid->getDX()-Ox;
                yloc = yfirst + j * grid->getDY()-Oy;
...
if (nmessageProj > 0) {
    ...
    for (i=1;i<lastindicex;i++){
        for (j=1;j<lastindicey;j++){
            reducedEx[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Ex[i][j][0];
            reducedEy[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Ey[i][j][0];
            reducedEz[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Ez[i][j][0];
            reducedBxn[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Bxn[i][j][0];
            reducedByn[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Byn[i][j][0];
            reducedBzn[ixsentProj[i]+1][iysentProj[j]+1][0] += weightProj[i][j][3][0]*Bzn[i][j][0];
            reducedEx[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Ex[i][j][0];
            reducedEy[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Ey[i][j][0];
            reducedEz[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Ez[i][j][0];
            reducedBxn[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Bxn[i][j][0];
            reducedByn[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Byn[i][j][0];
            reducedBzn[ixsentProj[i]+1][iysentProj[j]][0] += weightProj[i][j][2][0]*Bzn[i][j][0];
            reducedEx[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Ex[i][j][0];
            reducedEy[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Ey[i][j][0];
            reducedEz[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Ez[i][j][0];
            reducedBxn[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Bxn[i][j][0];
            reducedByn[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Byn[i][j][0];
            reducedBzn[ixsentProj[i]][iysentProj[j]+1][0] += weightProj[i][j][1][0]*Bzn[i][j][0];
            reducedEx[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Ex[i][j][0];
            reducedEy[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Ey[i][j][0];
            reducedEz[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Ez[i][j][0];
            reducedBxn[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Bxn[i][j][0];
            reducedByn[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Byn[i][j][0];
            reducedBzn[ixsentProj[i]][iysentProj[j]][0] += weightProj[i][j][0][0]*Bzn[i][j][0];
        }
    }
...
eqValue (0.0, Bzn_recvbufferproj, nxn,nyn);
    ...
    for (i=0;i<nmessagerecvProj;i++) {
    //cout << "Proc "<< vct->getCartesian_rank_COMMTOTAL() << "receives "<< npointsreceivedProj[i]<< " points from " << fromProj[i] <<endl;
...
for (i=ixrecvfirstProjglobal ;i<ixrecvfirstProjglobal+nxmrecv;i++){
    for (j=iyrecvfirstProjglobal;j<iyrecvfirstProjglobal+nymrecv;j++){
