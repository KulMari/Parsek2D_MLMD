...
coplot(xx,yy,uuu,rrhot,'x/d_i','y/d_i',['u_{e}(\omega_{ci}t=' num2str(time(it)) ')'])
F1(indexf)=getframe(gcf);

h=figure(2)
set(h,'Position' , [5 550 560 420]);
coplot(xx,yy,rrhot,rrhot,'x/d_i','y/d_i',['\rho (\omega_{ci}t=' num2str(time(it)) ')'])
F2(indexf)=getframe(gcf);

h=figure(3)
%set(h,'Position' , [565 5 560 420]);
set(h,'Position',[5 5 560 420]);
coplot(xx,yy,bbx.^2 + bby.^2 + bbz.^2,rrhot,'x/d_i','y/d_i',['B module(\omega_{ci}t=' num2str(time(it)) ')'])
F3(indexf)=getframe(gcf);

h=figure(4)
%set(h,'Position' , [565 550 560 420]);
set(h,'Position', [5 550 560 420]);
coplot(xx,yy,bbx,rrhot,'x/d_i','y/d_i',['B_x(\omega_{ci}t=' num2str(time(it)) ')'])
F4(indexf)=getframe(gcf);

h=figure(5)
set(h,'Position' , [1125 550 560 420]);
coplot(xx,yy,bby,rrhot,'x/d_i','y/d_i',['B_y(\omega_{ci}t=' num2str(time(it)) ')'])
...
movie2avi(F2,[name num2str(2) 'u.avi'],'fps',[4],'quality',[100])
movie2avi(F3,[name num2str(3) 'rho_net.avi'],'fps',[4],'quality',[100])
movie2avi(F4,[name num2str(4) 'Bx.avi'],'fps',[4],'quality',[100])
movie2avi(F5,[name num2str(5) 'By.avi'],'fps',[4],'quality',[100])
!ffmpeg -i film_mms1.avi -sameq -r 24 film_mms1.mpg
