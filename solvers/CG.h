...
inline bool CG(double *xkrylov, int xkrylovlen, double *b, int maxit, double tol, FIELD_IMAGE FunctionImage, Grid *grid, VirtualTopology *vct,Field *field){
    ...
    // v = r
    eq(v,r,xkrylovlen);
    c = dotP(vct->getCART_COMM(),r,r,xkrylovlen);
	...
	(field->*FunctionImage)(z,v,grid,vct);
        t = c/dotP(vct->getCART_COMM(),v,z,xkrylovlen);
	// x(i+1) = x + t*v
        addscale(t,xkrylov,v,xkrylovlen);
	// r(i+1) = r - t*z
        addscale(-t,r,z,xkrylovlen);
        d = dotP(vct->getCART_COMM(),r,r,xkrylovlen);
...
inline bool CG(double *xkrylov, int xkrylovlen, double *b, int maxit, double tol, GENERIC_IMAGE FunctionImage, Grid *grid, VirtualTopology *vct){
    // allocate residual, image, p, b, calculated on central points
    double *r    = new double[xkrylovlen];
    double *v    = new double[xkrylovlen];
    double *z    = new double[xkrylovlen];
