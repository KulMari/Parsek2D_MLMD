...
// r = b  - A*x
   ...
   initial_error = normP(vct->getCART_COMM(),r,xkrylovlen);
   normb= normP(vct->getCART_COMM(),b,xkrylovlen);
   if (normb == 0.0)
   normb = 1.0;
   
   if (itr==0){
    if (vct->getCartesian_rank() ==0)
    cout << "Initial residual: " << initial_error << " normb = "<<normb<<endl;
...
// w= A*V(:,k)
      ...
      av =  normP(vct->getCART_COMM(),w,xkrylovlen);
    
      for (register int j = 0; j <= k; j++) {
      getColumn(v,V,j,xkrylovlen);
      H[j][k] = dotP(vct->getCART_COMM(),w,v,xkrylovlen);
      addscale(-H[j][k],w,v,xkrylovlen);
   
      }
     putColumn(V,w,k+1,xkrylovlen); 
     H[k+1][k] = normP(vct->getCART_COMM(),w,xkrylovlen);
   
     if (av+delta * H[k+1][k] == av){
   
         for (register int j=0; j<=k; j++){
	      getColumn(v,V,j,xkrylovlen);
	      htmp=dotP(vct->getCART_COMM(),w,v,xkrylovlen);
	      H[j][k]= H[j][k]+htmp;
     ...
     if (0<k){ 
      ...
      if ( initial_error <= rho_tol) {
        if (vct->getCartesian_rank()==0)
      cout<<"GMRES converged on level " << grid->getLevel() << "  at restart # "<< itr <<"; iteration #"<< k <<" with error: "<<initial_error / rho_tol * tol <<endl;
        delete[] r;
   	delete[] im;
	delete[] s;
	delete[] v;
...
if (vct->getCartesian_rank()==0 )
cout<<"GMRES not converged on level" << grid->getLevel() << " !! Final error: "<<initial_error / rho_tol * tol <<endl;
