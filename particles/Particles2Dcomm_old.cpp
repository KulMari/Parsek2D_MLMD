...
void Particles2Dcomm::allocate(int species, CollectiveIO* col, VirtualTopology* vct, Grid* grid){
  ...
  npmax =  2*col->getNpMax(species); // for max test
  ...
  //buffer_size=(int) (.05*nop*nVar+1);//normal //(int) (10*nop*nVar+1); // max test
  //buffer_size = 10;
  ...
  max_np_SplitPartComm= (int) (.05*nop+1);//MAX_NP_REPOP_SIZE*  col->getRatio();  // the SplittedParticles_Comm_xxx buffers have originally size= max_np_SplitPartComm* nVar; their size can be increased during the simulation up to MAX_NP_SPLIPARTCOMM* nVar
  // putting a relatively small number in max_np_SplitPartComm allows self-tuning of the SP buffer size
  MAX_NP_SPLIPARTCOMM = MAX_NP_REPOP_SIZE*  col->getRatio()* col->getRatio();
...
/*for (int i=0; i< nmessageBC; i++)
  {
    if (BCSide[i]==3 && ns==0)
    {
      cout <<"R" << vct->getCartesian_rank_COMMTOTAL() << "sends BC right to R" << targetBC[i] <<" ns "<<ns <<endl;
    }
  }

for (int i=0; i<nmessagerecuBC; i++)
  {
    if (BCSidecu[i]==3 && ns ==0)
      {
	cout <<"R" << vct->getCartesian_rank_COMMTOTAL() << "receives BC right from R" << fromBC[i] <<" ns "<<ns<<endl;
      }
      }*/
...
int Particles2Dcomm::PRARepopulationAdd(int PN)
{
  ...
  if ( y[PN]>= PRA_CoyStartLeft && y[PN]<= PRA_CoyEndLeft && x[PN]>= PRA_CoxStartLeft && x[PN] <= PRA_CoxEndRight ) // area 0, Bottom
	...
	{
	  if (REPOP_b_BOTTOM[i*nVar +11]== ParticleID[PN] && qom== -1)
	    {
	      cout <<"Faulty ID " << ParticleID[PN] << endl;
	    }
	    }*/
	...
	REPOP_b_BOTTOM[np_REPOP_b_BOTTOM*nVar +11]= ParticleID[PN];
      ...
      /*if (qom== -1)
	cout  << "REPOP_b_BOTTOM "<< ParticleID[PN]<<endl; */
	...
	{
          if (REPOP_b_TOP[i*nVar +11]== ParticleID[PN] && qom== -1)
	    {
	      cout <<"Faulty ID " << ParticleID[PN] << endl;
            }
	    }*/
	...
	REPOP_b_TOP[np_REPOP_b_TOP*nVar +11]= ParticleID[PN];
      ...
      /*if (qom== -1)
        cout  << "REPOP_b_TOP "<< ParticleID[PN]<<endl;*/
	...
	{
          if (REPOP_b_LEFT[i*nVar +11]== ParticleID[PN] && qom== -1)
            {
              cout <<"Faulty ID " << ParticleID[PN] << endl;
            }
	    }*/
      ...
      /*if (qom== -1)
        cout << "REPOP_b_LEFT "<< ParticleID[PN]<<endl;*/
	...
	{
          if (REPOP_b_RIGHT[i*nVar +11]== ParticleID[PN] && qom== -1)
            {
              cout <<"Faulty ID " << ParticleID[PN] << endl;
            }
	    }*/
...
int Particles2Dcomm::PRASend(Grid* grid, VirtualTopology* vct)
{
  ...
  /*// TO REMOVE
  if (vct->getCartesian_rank_COMMTOTAL()==0)
    cout << "Level 0 before the barrier at the beginnign of PRASend\n";
  MPI_Barrier(vct->getCART_COMM());
  if (vct->getCartesian_rank_COMMTOTAL()==0)
    cout << "Level 0 after the barrier at the beginnign of PRASend\n";
    // END TO REMOVE*/
  ...
  if (!test_without_part)
    {
      if (PshareBOTTOM >np_REPOP_b_BOTTOM || PshareTOP >np_REPOP_b_TOP || PshareLEFT >np_REPOP_b_LEFT || PshareRIGHT >np_REPOP_b_RIGHT)
	{
  ...
  /*  MPI_Barrier(vct->getCART_COMM());
  if (vct->getCartesian_rank_COMMTOTAL()==0)
  cout << "Level 0 after the barrier in PRASend\n";*/
...
int Particles2Dcomm::communicateSP(VirtualTopology* ptVCT, Grid* grid){ 
  ...
  while (new_np_SplitPartComm > max_np_SplitPartComm){        
    cout << "R" << ptVCT->getCartesian_rank_COMMTOTAL() << "communicateSP resizing the receiving buffer" << endl;                            
    if (!resize_buffersSP((int) (new_np_SplitPartComm*1.5)))    
      {                                                                                                                                      
        cout<<"communicateSP: increase MAX_NP_SPLIPARTCOMM\nExiting...";                                                                     
        return -1;                                                                                                                           
      }
    cout << "End R" << ptVCT->getCartesian_rank_COMMTOTAL() << "communicateSP doubling the sending buffer size" << endl;                                                                                                                                      
  }
  ...
  if (npExitingMax > 0){
    ...
    communicateParticles(new_np_SplitPartComm*nVar,SplittedParticles_Comm_LEFT,SplittedParticles_Comm_RIGHT,SplittedParticles_Comm_BOTTOM, SplittedParticles_Comm_TOP,ptVCT);
...
int Particles2Dcomm::CountPRAParticles(VirtualTopology* vct)
{
  ...
  if (0 && vct->getCartesian_rank_COMMTOTAL() == vct->getXLEN()*vct->getYLEN())
    {
...
int Particles2Dcomm::communicateOS(VirtualTopology* ptVCT, Grid* grid)
{
  ...
  while (new_OSpart > max_np_OsPartComm){
    cout << "R" << ptVCT->getCartesian_rank_COMMTOTAL() << "communicateOS resizing the receiving buffer" << endl; 
    if (!resize_buffersOS((int) (new_OSpart*1.5)))
      {                                                         
        cout<<"communicateOS: increase MAX_NP_OSPARTCOMM\nExiting...";    
        return -1;                                                  
      } 
