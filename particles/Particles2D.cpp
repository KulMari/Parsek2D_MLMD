...
<li> dim = 2 --> constant velocity on Z direction </li>we
</ul>
...
int Particles2D::maxwellian_sameParticleInit(Grid* grid,Field* EMf,VirtualTopology* vct){
  ...
  if ( vct->getCartesian_rank_COMMTOTAL() % (vct->getXLEN()*vct->getYLEN())==0  )
	...
	for (int j=ycS; j< ycE;j++)
	  for (int ii=0; ii < npcelx; ii++)
	    for (int jj=0; jj < npcely; jj++){
	      ...
	      //cout << "R" <<vct->getCartesian_rank_COMMTOTAL() << "in maxwellian_sameParticleInit x[counter] " << x[counter] << ", y[counter] " << y[counter] <<endl;
	      // q = charge
	      //q[counter] =  (qom/fabs(qom))*(EMf->getRHOcs(i,j,0,ns)/npcel)*(1.0/invVOL);
    ...
    {
      nop=0;  // particles are not generated in the other procs
    }
  
  MPI_Barrier(vct->getCART_COMM());
  if ( ! (vct->getCartesian_rank_COMMTOTAL()%(vct->getXLEN()*vct->getYLEN()))   )
    {
      cout << "Level " << grid->getLevel() <<" arrived at the end of the particle generation part\n";
    }
  ...
  cout <<"R" << vct->getCartesian_rank_COMMTOTAL()<< " after communicate in maxwellian_SameParticleInit nop "<<nop <<"\n";
  MPI_Barrier(vct->getCART_COMM());
  if ( ! (vct->getCartesian_rank_COMMTOTAL()%(vct->getXLEN()*vct->getYLEN()))   )
    {
      cout << "Level " << grid->getLevel() <<" is resetting buffer sized in maxwellian_SameParticleInit\n";
    }
  // now, resizing everything to have vectors decently sized

  cout << "Setting decent buffer_size\n";
  buffer_size=(int) (.05*nop*nVar+1);

  if ((int) (npmax/ (vct->getNprocs())) > nop)
    {
      cout << "Setting decent npmax\n";
      npmax = (int) (npmax/ (vct->getNprocs())) ;
    }
