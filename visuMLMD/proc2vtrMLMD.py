...
"""
"""
    MODIFIED by M.E. Innocenti for Parsek2D_MLMD: 2D & multigrid
...
directory = '/ccc/scratch/cont005/ra0747/innocenm/MLMD/Smoothing/One_Level_Smooth0_Nvolte3/' 
...
def writePVTR(pvtr_name, cycle, vtr_prefix, scalar_fields, vector_fields, Nxc, Nyc, Nzc, numproc):
    ...
    for i in xrange(numproc):
        vtr_name = vtr_prefix + "_grid" +repr(numgrid)+ "_"+ repr(cycle).rjust(8,"0") + "-" + repr(i) + ".vtr"
        piece = pvtr.createElementNS("VTK", "Piece")
        piece.setAttribute("Extent", repr(i*Nxc/numproc) + " " + repr((i+1)*Nxc/numproc) + " 0 " + repr(Nyc) + " " + repr(0) + " " + repr(1)) #2D, with parallelization in X
...
ZLEN= 1# 2D
dx = h5setting_file.root.collective.Dx.read()[0] #this is printed from collective, so it's the value of the coarse grid; treated later
...
#MLMD
Ngrids = h5setting_file.root.collective.Ngrids.read()[0]
Ratio = h5setting_file.root.collective.Ratio.read()[0]
proc_filename1= directory + "proc" + repr(XLEN*YLEN*Level) + ".hdf"
h5proc_file1 = tables.openFile(proc_filename1, mode = "r", title = "Proc_file")
Ox=h5proc_file1.root.topology.Ox.read()[0]            
Oy=h5proc_file1.root.topology.Oy.read()[0]
h5proc_file1.close()
print "after first reading: Ngrids "+ str(Ngrids) + " Ratio " + str(Ratio) + " Ox "+str(Ox) + " Oy "+ str(Oy) 
#end MLMD
...
#2D, parallel in Y
Nzlocal =0
numgrid = Level #Number of the grid this proc is working on 
...
if len(list_read_proc) > 0:
    ...
    dy= dy/Ratio**numgrid
    #end MLMD
    print "Ox " + str(Ox) + " Oy " + str(Oy) + " dx " + str(dx)  + " dy " +str(dy) 
    Xcoordinates = Ox + scipy.arange(Nxc+1).flatten().astype('float32')*dx
    Ycoordinates = Oy + scipy.arange(Nyc+1).flatten().astype('float32')*dy
    ...
    if data_read['Fields']: 
        ...
        for cycle in list_cycle:    
                        ...
                        if type(node) == tables.array.Array:
                            ...
                            if num == cycle:
                                ...
                                """if rankproc == 1:
                                    print "node.read(): "
                                    print str(node.read())
                                    print "Fields_local[c, bounds[0]:bounds[1], bounds[2]:bounds[3], bounds[4]:bounds[5]]"
                                    print Fields_local[c, bounds[0]:bounds[1], bounds[2]:bounds[3], bounds[4]:bounds[5]]"""
                                #end check print
                        #end if                                                                                                        
                    #end for                                                                                                           
            ...
            for c in xrange(len(field_vec_names)): 
                ...
                writeVTR(directory + str_fields  + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0") + "-" + repr(rankproc)  + ".vtr", {}, vector_fields, vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeZlocal) #single grid 
            if rankproc == 0: #single grid
                pvtr_fields_list.append(str_fields + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0")+  ".pvtr")
    ...
    if data_read['Rho']: 
        ...
        for cycle in list_cycle:    
                ...
                for node in h5proc_file.walkNodes('/' + str_moments + '/' + str_rho):
                    ...
                    if type(node) == tables.array.Array:
                        ...
                        if num == cycle:        
                            ...
                            #old Parsek2D_MLMD used to print rhoc, so here problems with dimensions; now, it is printing rhon
                    #end if
                #end for                
            ...
            writeVTR(directory + str_rho + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0") + "-" + repr(rankproc) + ".vtr", rho_fields, {},
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeZlocal)
            if rankproc == 0:
                pvtr_rho_list.append(str_rho + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0") + ".pvtr")
    ...
    if data_read['Currents']:
        ...
        for cycle in list_cycle:    
            ...
            writeVTR(directory + str_currents+ "_grid" +repr(numgrid)+ "_"  + repr(cycle).rjust(8,"0") + "-" + repr(rankproc) + ".vtr", {}, j_fields,
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeZlocal)
            if rankproc == 0:
                pvtr_currents_list.append(str_currents + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0")+".pvtr")
    ...
    if data_read['Pressure']: 
        ...
        for cycle in list_cycle:    
            ...
            #end for
            writeVTR(directory + str_pressure + "_grid" +repr(numgrid)+ "_"  + repr(cycle).rjust(8,"0") + "-" + repr(rankproc) + ".vtr", p_fields, {},
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeZlocal)
            if rankproc == 0:         
                pvtr_press_list.append(str_pressure + "_grid" +repr(numgrid)+ "_" + repr(cycle).rjust(8,"0") + ".pvtr")
