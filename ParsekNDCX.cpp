/********************************************************************************************
ParsekNDCX.cpp  - A main file to siumlate the NDCX experiment - beam neutralized in plasma
			        -------------------
 ...
 if (nprocs != vct->getNprocs()){ // Check if we can map the processes into a matrix ordering defined in Collective.cpp
  ...
  for (int cycle = first_cycle; cycle < (col->getNcycles()+first_cycle); cycle++){
	  ...
	  for (int i=0; i < ns; i++) // move each species
	   ...
	   if (cycle > 50){  // start injecting after 50 cycles
	        if (myrank==0){
		         cout << "Injecting particles from the X-left Wall" << endl;
			}
			//put the injected beam with electrons
			part[0].inject_beam(0.0,5.0,0.1,0.05,0.0,0.05,0.0,0.05,2,500,0.1,vct); // inject with uniform density and v = .1 
