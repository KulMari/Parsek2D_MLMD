...
inline double dotP(MPI_Comm CART_COMM, double *vect1, double *vect2,int n){
  double result = 0;
  double local_result = 0;
  for (register int i=0; i< n; i++)
      local_result += vect1[i]*vect2[i];
  MPI_Allreduce(&local_result, &result, 1, MPI_DOUBLE, MPI_SUM, CART_COMM);
  return(result);
...
inline double norm2P(MPI_Comm CART_COMM, double ***vect, int nx, int ny, int nz){
  double result = 0;
  double local_result = 0;
  for (int i=0; i<nx; i++)
      for (int j=0; j<ny; j++)
...
inline double norm2P(MPI_Comm CART_COMM, double *vect, int n){
  double result = 0;
  double local_result = 0;
  for (int i=0; i< n; i++)
...
inline double normP(MPI_Comm CART_COMM, double *vect, int n){
  double result = 0;
  double local_result = 0;
  for (register int i=0; i<n; i++)
      local_result += vect[i]*vect[i];
...
/** method to calculate vector1 = beta*vector1 + alfa*vector2 */
inline void addscale_no_BC(double alfa, double beta, double ***vect1, double ***vect2,int nx, int ny){
