...
namespace PSK {
   ...
   public:
      OutputException( const std::string& err_str, const std::string fn_str="",
                   int sys_errno = 0 ) : Exception( err_str, fn_str, sys_errno ){
...
*/
   class Dimens {
       ...
       public:
          ...
          Dimens( int d1, int d2 ) : _dimens(2) { _dimens[0]=d1; _dimens[1]=d2;}
          Dimens( int d1, int d2, int d3 ): _dimens(3) { _dimens[0]=d1; _dimens[1]=d2; _dimens[2]=d3; }
          Dimens( int d1, int d2, int d3, int d4 ): _dimens(4) { _dimens[0]=d1; _dimens[1]=d2;
...
public:
  ...
  virtual void write( const std::string& objname,
    ...
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(int* array)");}
  virtual void write( const std::string& objname,
			  const Dimens dimens,
			   const long* i_array )
     { throw OutputException("Function not implemented",
				      "PSK::OutputAdaptor::write(long* array)");}
 virtual void write( const std::string& objname,
                        const Dimens dimens,
                        const std::vector<int>& i_array )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(vector<int> array)");}

virtual void write( const std::string& objname,
			  const Dimens dimens,
			  const std::vector<long>& i_array )
      { throw OutputException("Function not implemented",
				      "PSK::OutputAdaptor::write(vector<long> array)");}
// write float functions
  virtual void write( const std::string& objname, float f )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(float)");}
  virtual void write( const std::string& objname,
                        const Dimens dimens,
                        const float* f_array )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(float* array)");}
  virtual void write( const std::string& objname,
                        const Dimens dimens,
                        const std::vector<float>& f_array )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(vector<float> array)");}

// write double functions
  virtual void write( const std::string& objname, double d )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(double)");}
  virtual void write( const std::string& objname,
                        const Dimens dimens,
                        const double* d_array )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(double* array)");}
  virtual void write( const std::string& objname,
                        const Dimens dimens,
                        const std::vector<double>& d_array )
    { throw OutputException("Function not implemented",
              "PSK::OutputAdaptor::write(vector<double> array)");}
  ...
  template <class Toa> class OutputAgent : public OutputAgentBase {
    ...
    public:
      ...
      void open_append( const std::string& outf ) { output_adaptor.open_append( outf ); }
...
public:
	...
	void set_simulation_pointers(Field* field, Grid* grid, VCtopology* vct, MPIdata* mpi, Collective* col)	{
		_field= field;_grid = grid;_vct= vct;_mpi=mpi;_col=col;
	}
...
void output(const string& tag, int cycle){
	...
	if (tag.find("collective",0) != string::npos){
		...
		this->output_adaptor.write ("/collective/Nvolte",_col->getNvolte() );
		this->output_adaptor.write ("/collective/Ngrids",_col->getNgrids() );
		this->output_adaptor.write ("/collective/Ratio",_col->getRatio() );

		this->output_adaptor.write ("/collective/B0x",_col->getB0x() );
		this->output_adaptor.write ("/collective/B0y",_col->getB0y() );
		this->output_adaptor.write ("/collective/B0z",_col->getB0z() );

		this->output_adaptor.write ("/collective/Vinj",_col->getVinj() );
	...
	if (tag.find("proc_topology",0) != string::npos){
		...
		this->output_adaptor.write ("/topology/cartesian_coord",PSK::Dimens(2),coord);
		...
		this->output_adaptor.write ("/topology/Level",_grid->getLevel () );
		this->output_adaptor.write ("/topology/Ox",_grid->getOx (_grid->getLevel() ) );
		this->output_adaptor.write ("/topology/Oy",_grid->getOy (_grid->getLevel() ) );
		delete[] coord;
	}

        // Bfield is written without ghost cells and defined in nodes
	if (tag.find("Ball",0) != string::npos){
		this->output_adaptor.write ("/fields/Bx/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getBx());
		this->output_adaptor.write ("/fields/By/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getBy());
		this->output_adaptor.write ("/fields/Bz/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getBz());
	}
...
// Efield is written without ghost cells and defined in nodes
       ...
       if (tag.find("Eall",0) != string::npos){
		this->output_adaptor.write ("/fields/Ex/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getEx() );

		this->output_adaptor.write ("/fields/Ey/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getEy() );
		this->output_adaptor.write ("/fields/Ez/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getEz() );
	}
...
// PHI is written without ghost cells and defined in centers
	if (tag.find("phi",0) != string::npos){
		this->output_adaptor.write ("/potentials/phi/cycle_"+cc.str(), PSK::Dimens(_grid->getNXC()-2 ,_grid->getNYC()-2,1),_field->getPHI() );
		}
...
// J (current density) is written without ghost cells and defined in nodes
		if (tag.find("Jall",0) != string::npos){
			...
			this->output_adaptor.write ("/moments/Jx/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getJx() );
			this->output_adaptor.write ("/moments/Jy/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getJy() );
			this->output_adaptor.write ("/moments/Jz/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getJz() );
		}
...
// Js (current density for species s) is written without ghost cells and defined in nodes
		if (tag.find("Jsall",0) != string::npos){
			for ( int i=0; i<ns; ++i ){
				stringstream ii;
				ii << i;
				this->output_adaptor.write ("/moments/species_"+ii.str()+"/Jx/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getJxs() );
				this->output_adaptor.write ("/moments/species_"+ii.str()+"/Jy/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getJys() );
				this->output_adaptor.write ("/moments/species_"+ii.str()+"/Jz/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getJzs() );
			} 	}
...
// rhos (number density for species s) is written without ghost cells and defined in nodes
		if (tag.find("rhos",0) != string::npos){
				...
				stringstream ii;
				ii << i;
				this->output_adaptor.write ("/moments/species_"+ii.str()+"/rho/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getRHOns() );
				} 	}

//// rho (number density ) is written without ghost cells and defined in centres
		//if (tag.find("rho",0) != string::npos){
                //       		this->output_adaptor.write ("/moments/rho/cycle_"+cc.str(), PSK::Dimens(_grid->getNXC()-2 ,_grid->getNYC()-2,1),_field->getRHOc() );
		//	 	}
// rho (number density ) is written without ghost cells and defined in nodes (changed with respect to before)                                                                        
		if (tag.find("rho",0) != string::npos){                                                                                                                    
                                      this->output_adaptor.write ("/moments/rho/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),_field->getRHOn() );
                              } 
// potential is written without ghost cells and defined in nodes
		if (tag.find("phi",0) != string::npos){
                       		this->output_adaptor.write ("/moments/phi/cycle_"+cc.str(), PSK::Dimens(_grid->getNXC()-2 ,_grid->getNYC()-2,1),_field->getPHI() );
			 	}
// pressure for species s is written without ghost cells and defined in nodes
		if (tag.find("pressure",0) != string::npos){
			...
			stringstream ii;
			ii << i;
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pXX/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpXXsn() );
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pXY/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpXYsn() );
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pXZ/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpXZsn() );
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pYY/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpYYsn() );
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pYZ/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpYZsn() );
			this->output_adaptor.write ("/moments/species_"+ii.str()+"/pZZ/cycle_"+cc.str(), PSK::Dimens(_grid->getNXN()-2 ,_grid->getNYN()-2,1),i,_field->getpZZsn() );
			} 	}
...
// Particle ID
		...
		if (tag.find("ID",0) != string::npos & sample == 0){
			...
			else if (_vct->getCartesian_rank()==0) cout<<"Can't write particle ID for species "+ii.str()+" because TrackParticleID is = false "<<endl;
			} }
