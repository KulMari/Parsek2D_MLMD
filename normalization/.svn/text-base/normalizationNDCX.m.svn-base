...
%% program to calculate plasma parameters for simulation with PARSEK and NDCX
clear all;close all
...
n0=5.0e10; % background plasma
% Reference temperature - in eV
T=3.0;  % eV temperature plasma

% Density of the beam
n_beam=5.0e9;
% Current of the beam not in CGS!!!!! 25 milliampere
j_beam=25e-3; %this is probably to get the density
% Temperature of the beam
T_beam = 300.0e3 % 300 KeV
% cm/s
v_beam = 9.79e5*sqrt(2*T_beam)
% find the density of the beam from the current
n_beam= (j_beam/((1.60217646e-19)*v_beam*1e-2))*1e-6
%(j_beam/((1.60217646e-19)*n_beam*1e6))*100
% Temperature of the beam
%T_beam = 300.0e3 % 300 KeV
...
L_pipe = 130; % pipe length 1 meter
radius_pipe = 3.8; % pipe radius 7 cm
radius_beam = 2; % beam radius 2 cm     

% we want to solve the Debye length for the sheath
Dx = ld;    
Nx = L_pipe/Dx;
Ny = 2*radius_beam/Dx;
% time of the simulation is approximately time in the chamber of the beam
...
L_pipe_normalized=L_pipe/norm_space;
radius_pipe_normalized = radius_pipe/norm_space;
radius_beam_normalized = radius_beam/norm_space;
